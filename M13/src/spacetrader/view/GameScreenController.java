...
import spacetrader.apis.Encounter;
/**
...
public class GameScreenController implements Initializable, ControlledScreen {
    ...
    private void loadTargetInfo() {
         //target system
        String targetName = "Name:  " + targetSystem.getName();
    ...
    private void loadCurrentInfo() {
         //target system
        String currName = "Name:  " + solarSystem.getName();
        //String capSize = "Name  " + cap.get
    ...
    private void warpFired(ActionEvent event) {
    	if (!findPlanetField.textProperty().get().isEmpty()) {
    		encounterCount = 0;
        	loadEncounterScreen();
    	}
    ...
    private void travel(int hullCost) {
        ...
        if (!targetSystem.getName().equals(solarSystem.getName())) {
        	travel.warpTo(targetSystem.getName(), travelDist, hullCost);
    ...
    private void loadEncounterScreen() {
    	Stage newStage = new Stage();
    	VBox comp = new VBox();
    	Encounter en = new Encounter(SqliteApi.getShip());
    	en.meet();
    	Image encounterShipImg = new Image(en.getOppShip().getUrl());
    	ImageView imgView = new ImageView();
    	imgView.setImage(encounterShipImg);
    	
    	int typeNum = en.getOppType();
    	Label typeInfo;
    	if (typeNum == 0) {
    		typeInfo = new Label("Police");
    	} else {
    		typeInfo = new Label("Pirate");
    	}
    	String s = en.getOppAction(typeNum);
    	if (en.isDead()) {
    		System.out.println("You are killed.");
    		myController.setScreen(Main.screen1ID);
    	}
    	System.out.println(s);
    	Label actionInfo = new Label(s);
    	PlayerShip ps = en.getPlayerShip();
    	Ship os = en.getOppShip();
    	Label shipType = new Label("Opponent Ship Type: " + en.getOpShipType());
    	Label otherHullInfo = new Label("Opponent Hull Strength: " + os.getHullStrength());
    	Label hullInfo = new Label("Your Hull Strength: " + ps.getBase().getHullStrength());
    	...
    	attackButton.setOnAction(new EventHandler<ActionEvent>() {
    		...
    		public void handle(ActionEvent e) {
    			if (encounterCount == 4) {
    				newStage.close();
    				travel(SqliteApi.getShip().getBase().getHullStrength() - en.getPlayerShip().getBase().getHullStrength());
        			...
        			int typeNum = en.getOppType();
        			if (typeNum == 0) {
        				typeInfo.setText("Police");
        				
        			} else {
        				typeInfo.setText("Pirate");
        			}
        			String s = en.getOppAction(typeNum);
        			if (en.isDead()) {
        	    		System.out.println("You are killed.");
        	    	...
        	    	System.out.println(s);
        	    	actionInfo.setText(s);
        	    	PlayerShip ps = en.getPlayerShip();
        	    	Ship os = en.getOppShip();
        	    	shipType.setText("Opponent Ship Type: " + en.getOpShipType());
        	    	otherHullInfo.setText("Opponent Hull Strength: " + os.getHullStrength());
        	    	hullInfo.setText("Your Hull Strength: " + ps.getBase().getHullStrength());
        			encounterCount++;
    			}
    	...
    	comp.getChildren().add(imgView);
    	comp.getChildren().add(typeInfo);
    	comp.getChildren().add(shipType);
    	comp.getChildren().add(otherHullInfo);
    	comp.getChildren().add(actionInfo);
    	comp.getChildren().add(new Label(""));
    	comp.getChildren().add(hullInfo);
    	comp.getChildren().add(attackButton);
    	comp.getChildren().add(new Label(""));
    	comp.getChildren().add(fleeButton);
    ...
    private void updatePlayerInfo() {
			...
			String s1 = "Cash: " + SqliteApi.getPlayer().getMoney() + " cr";
			String s2 = "Ship Type: " + SqliteApi.getShip().getBase().getName();
	        String s3 = "Attack: " + SqliteApi.getShip().getAttack() + "  Shield: " + SqliteApi.getShip().getShield();
	        String s4 = "Remaining Weapon Slots: " + SqliteApi.getShip().getWeaponSlots();
	        String s7 = "Remaining Shield Slots: " + SqliteApi.getShip().getShieldSlots();
	        String s5 = "Remaining Gadget Slots: " + SqliteApi.getShip().getGadgetsSlots();
	        String s6 = "Cargo Space Remaining: " + (SqliteApi.getShip().getCargoSpace() - SqliteApi.getShip().getCargo().size());
	        playerInfo = FXCollections.observableArrayList(
	         s1, s2, s3, s4, s7, s5, s6);
	        
	        for (Good g : SqliteApi.getShip().getCargo()) {
    ...
    private void setRefuelChoose() {
			...
			int currFuel = SqliteApi.getShip().getBase().getFuel();
			ShipFactory sf = new ShipFactory();
			Ship modelShip = sf.getShip(SqliteApi.getShip().getBase().getName());
    ...
    private void setRepairChoose() {
			...
			int currStre = SqliteApi.getShip().getBase().getHullStrength();
			ShipFactory sf = new ShipFactory();
			Ship modelShip = sf.getShip(SqliteApi.getShip().getBase().getName());
...
//    
    ...
    private void refuelFired(ActionEvent event) {
			...
			int perCost = SqliteApi.getShip().getBase().getFuelCost();
			int amount = converter.fromString(refuelChoose.getValue().toString());
			int cost = amount * perCost;
			
			player = SqliteApi.getPlayer();
			ship = SqliteApi.getShip();
			player.setMoney(SqliteApi.getPlayer().getMoney() - cost);
			ship.getBase().setFuel(SqliteApi.getShip().getBase().getFuel() + amount);
			SqliteApi.setPlayer(player);
			SqliteApi.setShip(ship);
			setRefuelChoose();
			// update dock info, redraw long and short range charts
			showDockInfo(SqliteApi.getShip(), SqliteApi.getShip().getBase().getFuel(), SqliteApi.getShip().getBase().getHullStrength());
			gc = canvas.getGraphicsContext2D();
		    drawLongRange(SqliteApi.getPlayer().getXpos(), SqliteApi.getPlayer().getYpos(), SqliteApi.getShip().getBase().getFuel(), gc);
		    gc2 = canvas2.getGraphicsContext2D();
		    drawShortRange(SqliteApi.getPlayer().getXpos(), SqliteApi.getPlayer().getYpos(), SqliteApi.getShip().getBase().getFuel(), gc2);
    ...
    private void repairFired(ActionEvent event) {
			int perCost = SqliteApi.getShip().getBase().getRepairCost();
    	...
    	for (Tab t : shipyardPane.getTabs()) {
    		if (t.isSelected() && t.getId().equals("shipTab")) {
    			...
    			if (shipNames.size() > 0) {
        	    	...
        	    	//System.out.println("player's current ship: " + SqliteApi.getShip().getBase().getName());
        	    	updatePlayerInfo();
        	    	showDockInfo(SqliteApi.getShip(), SqliteApi.getShip().getBase().getFuel(), SqliteApi.getShip().getBase().getHullStrength());
        			gc = canvas.getGraphicsContext2D();
        			drawLongRange(SqliteApi.getPlayer().getXpos(), SqliteApi.getPlayer().getYpos(), SqliteApi.getShip().getBase().getFuel(), gc);
    ...
    private void waterSellFired(ActionEvent event) {
        if (waterChoose.getValue() != null) {
            ...
            mess(toBuyList, toBuyMap, toSellList, toSellMap, mp);
            //updateDatabase(SqliteApi.getPlayer(), SqliteApi.getShip());
            updatePlayerInfo();
            sy = new ShipYard();
            showShipNames(sy);
       }
    ...
    private void showShipNames(ShipYard sy) {
    	...
    	ObservableList<String> gadgetItem = FXCollections.observableArrayList();
   	 	if (!sy.isYardExist()) {
   	 		...
   	 		if (weaponNames.size() == 0) {
   	 			if (SqliteApi.getShip().getWeaponSlots() <= 0) {
	 		...
	 		shieldPrices = sy.getShieldPrices();
   	 		if (shieldNames.size() == 0) {
   	 			if (SqliteApi.getShip().getShieldSlots() <= 0) {
   	 		...
   	 		int techLevel = 0;
   	 		try {
				techLevel = SqliteApi.getSolarSystem().getPlanet().getTechLevel().ordinal();
				if (techLevel == 6) {
	   	 			if (gadgetNames.size() == 0) {
	   	 				if (SqliteApi.getShip().getGadgetsSlots() <= 0) {
    ...
    public void initialize(URL url, ResourceBundle rb) {
    	try {
	        ...
	        ship = SqliteApi.getShip();
	        solarList = SqliteApi.getUniverse();
    ...
    public void mess(List<String> toBuyList, Map<String, Integer> toBuyMap, List<String> toSellList, Map<String, Integer> toSellMap, MarketPlace mp) {
        int maxAmount1;
        int maxAmount2;
        maxAmount1 = SqliteApi.getShip().getCargoSpace() - mp.getCargo().size();
        for (String s : toBuyList) {
            if (s.equals("Water")) {
                ...
                machinesPrice2.setText(Integer.toString(toBuyMap.get("Machines")));
